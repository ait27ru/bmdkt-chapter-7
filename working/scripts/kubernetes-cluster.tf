...
resource "azurerm_kubernetes_cluster" "cluster" {
    ...
    // to find out available k8s version: 
    // az aks get-versions --location westus --output table
    kubernetes_version  = "1.24.3"

    // set authentication datails for the cluster
    linux_profile {
    ...
    // configure the nodes for the cluster
    default_node_pool {
        name            = "default"
        node_count      = 1
    ...
    // configure authentication details to allow the cluster to interact with Azure
    service_principal {
